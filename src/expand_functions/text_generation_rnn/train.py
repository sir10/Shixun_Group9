...
import Config #配置文件路径和训练生成的一些参数
import Model
...
file = sys.argv[1]
data = open(file,'r', encoding='utf-8').read() #读文本文件
#data = data.decode('utf-8') #设置编码
...
cPickle.dump((char_to_idx, idx_to_char), open(config.model_path+'.voc','wb'), -1)
...
def data_iterator(raw_data, batch_size, num_steps):
    ...
    if epoch_size == 0: # 此处判定文本长度，如果长度太短训练效果不好会报错，需要增加文本素材的文字长度
...
def main(_):
    ...
    with tf.Graph().as_default(), tf.Session(config=config_tf) as session:
        ...
        if config.last_iteration > 0: # 读配置文件进行增量训练
            model_saver.restore(session, config.model_path+'-%d'%(config.last_iteration)) # 即将固化到硬盘中的Session从保存路径再读取出来
            print("Model Restored")

        for i in range(config.last_iteration,config.iteration):
