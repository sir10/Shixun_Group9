<template>
  <div class="com-container">
    <div class="com-chart" ref="test2_ref"></div>
  </div>
</template>
...
<script>
  export default {
    ...
    methods: {
      initChart () {
        this.chartInstance = this.$echarts.init(this.$refs.test2_ref, 'chalk')
        ...
        var color = ['#00ffff', '#00cfff', '#006ced', '#ffe000', '#35B7ED', '#F751FF', '#7E38ED']
      ...
      async getData () {
        const { data: ret } = await this.$http.get('travelpartner')
        this.allData = ret
      ...
      updateChart () {
        ...
        for (var i = 0; i < this.allData.length; i++) {
          data.push({
            value: this.allData[i].number,
            name: this.allData[i].partnertype,
            itemStyle: {
              normal: {
                borderWidth: 5,
                shadowBlur: 20,
